...
def load_market_data(symbol: str = "LEN", period: str = "2y"):
    ...
    # yfinance иногда возвращает MultiIndex даже для одного тикера.
    # Приводим Close к обычной Series, чтобы Streamlit получал обычный DataFrame.
...
try:
    market_df = load_market_data("LEN", "2y")
    lennar = yf.Ticker("LEN")
    info = lennar.info
...
try:
    market_df = load_market_data("LEN", period)
    lennar = yf.Ticker("LEN")
    info = lennar.info
except Exception as exc:
